describe('Farms', () => {
    ...
    it('Should open farms when clicked', () => {
        cy.get('#farms-table', { timeout: 20000 }).contains('td', 'Float').should('be.visible').click();
    ...
    it('Filters should work correctly', () => {
        ...
        cy.get('#anchor-select-tab').should('be.visible').click().should('have.class', 'ACTIVE')
        cy.get('#all-select-tab').should('not.have.class', 'ACTIVE')
        cy.get('#float-select-tab').should('be.visible').click().should('have.class', 'ACTIVE')
        cy.get('#anchor-select-tab').should('not.have.class', 'ACTIVE')
    })
