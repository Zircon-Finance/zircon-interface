...
export const PhishingBanner = () => {
  ...
  const openWebsite = () => {
    const newWindow = window.open('https://beta.diffuse.finance', '_blank', 'noopener,noreferrer')
  ...
  return (
    <PhyshingContainer>
        ...
        <Flex flexDirection={width <= 992 ? 'column' : 'row'} py={width <= 992 && '10px'}>
          ...
          <Link to={'#'} onClick={() => openWebsite()} style={{textDecoration: 'none', color: '#E9D886', marginLeft: width >= 992 && '5px'}}> https://app.diffuse.finance </Link>
