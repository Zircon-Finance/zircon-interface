...
export function useDerivedPylonBurnInfo(
  ...
  if (typedValue !== '0' && pylon !== undefined && userLiquidity !== undefined && independentField === Field.LIQUIDITY_PERCENT)  {
    userLiquidityAdjustedPercentage = new TokenAmount(isFloat ? pylon?.floatLiquidityToken : pylon?.anchorLiquidityToken, JSBI.divide(
      JSBI.multiply(
  ...
  const parsedAmounts: {
    ...
    [Field.LIQUIDITY]:
      ...
      new TokenAmount(userLiquidity.token, percentToRemove.multiply(userLiquidity.raw).quotient)
      : userLiquidity && percentToRemove && percentToRemove.greaterThan('0') && new TokenAmount(userLiquidity.token, percentToRemove.multiply(userLiquidity.raw).quotient)
            : undefined,
    [Field.CURRENCY_A]:
